...
В проекте реализован offline / batch мониторинг дрифта признаков для контроля стабильности входных данных модели.

**Зачем:**
В production-ML важно отслеживать, не изменились ли распределения признаков по сравнению с обучающей выборкой, так как это может привести к деградации качества модели.

**Подход**

- Reference dataset: обучающая выборка (train)

- Current dataset: новая партия данных (test)

- Уровень мониторинга: feature-level (до One-Hot Encoding)

- Метрика: Population Stability Index (PSI)

Для каждого признака считается PSI между reference и current распределениями.
...
PSI считается по квантильным бинам, рассчитанным на reference-данных.

- Категориальные признаки:

PSI считается по долям категорий, включая новые (unseen) значения.

- Дополнительно в отчёте фиксируются:

    - доля пропусков (reference / current),

    - количество уникальных категорий (для categorical).
...
После запуска формируется отчёт:

```
reports/drift_report.csv
```

**Отчёт содержит таблицу со следующими полями:**
...
- ref_unique / cur_unique (для categorical)

В текущем датасете значимого дрифта не обнаружено, что ожидаемо, так как train и test происходят из одного распределения.
